...
export const Config = z.object({
  ...
  oodaBackfillUrl: z.string().default('http://127.0.0.1:11435'), // 本地 4B 端点（ollama-priority-proxy 主入口，Fct 生成；调用带 X-Queue-Priority: normal/low）
  ...
  toolBackfillUrl: z.string().default('http://127.0.0.1:11435'), // 本地 4B 端点（ollama-priority-proxy 主入口，调用带 X-Queue-Priority: low）
  ...
  realityEmbedUrl: z.string().default('http://127.0.0.1:11435/api/embed'), // 本地 embedding 端点（主代理 /api/embed，代理路由到 Embed Worker；不用旧 11439）
  ...
  realityPickUrl: z.string().default('http://127.0.0.1:11435'), // 本地 4B 端点（ollama-priority-proxy 主入口，调用带 X-Queue-Priority: high；Hermes LLM_ENDPOINT）
